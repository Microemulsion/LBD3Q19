...
void init_random_seed(long seed)
{
  ...
  if(seed < 1) {
    ...
    fflush(NULL); //errexit();
	throw ComException("The initial seed of the random number generator must be a positive integer! \n Using 0 will result in a plain 0-sequence, hence it's forbidden! ", __FILE__, __LINE__ );
  }
  idumInit = idum = seed;
